...
test("September 19 has 54 slides and its own PDF and material entry", async () => {
  assert.equal(lesson2.date, "2026-09-19");
  assert.equal(lesson2Slides(false).length, 54);
  ...
  for (const locale of ["", "en/"]) {
    ...
    assert.equal((page.match(/id="slide-\d+"/g) || []).length, 54);
