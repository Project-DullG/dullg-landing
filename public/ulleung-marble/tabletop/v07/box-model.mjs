...
import {boxTextures} from './box-textures.mjs?v=20260923j';
...
export function createBox(state=0){
 ...
 const mats={teal:new THREE.MeshStandardMaterial({color:'#174d59',roughness:.83}),paper:new THREE.MeshStandardMaterial({color:'#f4ebd5',roughness:.94}),insert:new THREE.MeshStandardMaterial({color:'#dfd9c7',roughness:1}),edge:new THREE.MeshStandardMaterial({color:'#ae9877',roughness:.85})};
 ...
 const print=(g,key,x,y,z,w,h,vertical=false,turn=0)=>{const map=boxTextures.get(key);if(!map)return;const face=new THREE.Mesh(new THREE.PlaneGeometry(w,h),new THREE.MeshStandardMaterial({map,roughness:.88}));face.position.set(x,y,z);if(!vertical)face.rotation.x=-Math.PI/2;face.rotation.y=turn;g.add(face);};
 ...
 const base=new THREE.Group();root.add(base);shell(base,.376,.236,.066,mats.teal);print(base,'back',0,-.00005,0,.376,.236);base.children.at(-1).rotation.x=Math.PI/2;box(base,0,.003,0,.372,.002,.232,mats.insert);box(base,-.113,.0105,0,.138,.013,.224,mats.insert);box(base,.169,.0105,0,.022,.013,.110,mats.insert);
 ...
 // Shallow wells separate the wooden pieces without changing the box envelope.
 const well=new THREE.MeshStandardMaterial({color:'#c2cfc5',roughness:1});
 for(let i=0;i<8;i++){box(base,-.153+(i%4)*.029,.0171,-.06+Math.floor(i/4)*.040,.026,.0003,.034,well);}
 for(let i=0;i<8;i++)box(base,.169,.0171,-.038+i*.011,.014,.0003,.010,well);
 for(const x of [-.153,-.125])box(base,x,.0171,.104,.024,.0003,.024,well);
 ...
 print(lid,'side',0,.016,-.12004,.376,.029,true,Math.PI);
 print(lid,'end',-.19004,.016,0,.236,.029,true,-Math.PI/2);print(lid,'end',.19004,.016,0,.236,.029,true,Math.PI/2);
 // A light inner lid and an inventory panel are visible when the package is turned.
 const inner=box(lid,0,.029,0,.375,.0005,.235,mats.paper);
