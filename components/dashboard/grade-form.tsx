...
export function GradeForm({ students }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="dash-card">
      ...
      <label htmlFor="grade-student">학생</label>
      <select
        id="grade-student"
      ...
      {type === "dullg" ? (
        <>
          <label htmlFor="grade-session">수업 차시</label>
          <select
            id="grade-session"
            className="dash-select"
            value={session}
          ...
          <label htmlFor="grade-score">점수 (100점 만점)</label>
          <input
            id="grade-score"
            ...
            required
          />
          <label htmlFor="grade-participation">수업 참여도</label>
          <select
          ...
          <label htmlFor="grade-note">메모 (선택)</label>
          <input
            id="grade-note"
            className="dash-input"
            placeholder="메모 (선택)"
        ...
        <>
          <label htmlFor="grade-subject">과목</label>
          <input
            id="grade-subject"
            className="dash-input"
            placeholder="과목명"
            ...
            required
          />
          <label htmlFor="grade-exam">시험 이름</label>
          <input
            id="grade-exam"
            className="dash-input"
      ...
      <label htmlFor="grade-date">평가일</label>
      <input
        id="grade-date"
        className="dash-input"
        type="date"
      ...
      {success && (
        <p role="status" className="dash-success">성적을 저장했습니다. 다음 기록을 입력할 수 있습니다.</p>
      )}
    </form>
  );
}
