...
export function GameCover({ kind, title }: { kind: MiniGameId; title: string }) {
  const t = useText();
  if (!isArcadeGame(kind)) return <TableGameCover kind={kind} title={t(title)} />;
  return (
    <div className={`${styles.cover} ${styles[kind]}`}>
      <svg viewBox="0 0 400 310" role="img" aria-label={t(`${title} 게임 화면 구성`)}>
        {t(
          kind === "block-stack" ? (
            <>
            ...
            <>
              ...
              />
              <image
                href="/assets/games/racing/player.png"
                x="181"
                y="210"
                width="38"
                height="70"
