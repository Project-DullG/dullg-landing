...
const PREFIX='echo_detective_v1_';
const defaults={textSize:1,speed:12,instant:false,autoInterval:3500,volume:.35,musicVolume:.2,audioMuted:false,ambientVolume:.15,voiceVolume:.8,voice:false,voiceRate:1,reduced:false,hotspots:true,unfocusedMute:true,contrast:false,hints:true,controlHints:false,impactEffects:true,sceneEffects:true,atmosphere:true,characterMotion:true,textEffects:true,punctuationPause:true};
function sum(t){let h=2166136261;for(let i=0;i<t.length;i++){h^=t.charCodeAt(i);h=Math.imul(h,16777619);}return(h>>>0).toString(16);}
function encode(s,ui){const payload=JSON.stringify({game:s,ui:{mode:ui.mode,npc:ui.npc,scrolls:ui.scrolls||{},evidenceFilter:ui.evidenceFilter||'all',evidenceSearch:ui.evidenceSearch||'',caseTab:ui.caseTab||'overview',proofDrafts:ui.proofDrafts||{},compareDrafts:ui.compareDrafts||{},docViews:ui.docViews||{},returnContext:ui.returnContext||null,lastResult:ui.lastResult||null,moreQuestions:ui.moreQuestions||{},evidenceOrder:ui.evidenceOrder||'recent',loopTab:ui.loopTab||'facts',advancedCase:!!ui.advancedCase,pastJudgments:!!ui.pastJudgments,onboarding:ui.onboarding||{}},savedAt:new Date().toISOString()});return JSON.stringify({format:'echo-detective-save',version:2,checksum:sum(payload),payload});}
function decode(raw){let w,v;try{w=JSON.parse(raw);}catch(e){throw Error('저장 파일의 JSON 형식이 손상되었습니다.');}if(!w||!['last-screening-save','echo-detective-save'].includes(w.format)||![1,2].includes(w.version)||typeof w.payload!=='string'||w.checksum!==sum(w.payload))throw Error('저장 파일 형식이나 무결성을 확인할 수 없습니다.');try{v=JSON.parse(w.payload);root.MODEL.normalizeHistory?.(v.game);root.MODEL.validate(v.game);}catch(e){throw Error('저장 진행을 복원할 수 없습니다. '+e.message);}if(typeof v.savedAt!=='string'||!Number.isFinite(Date.parse(v.savedAt)))throw Error('저장 시각이 잘못되었습니다.');v.ui=v.ui&&typeof v.ui==='object'?v.ui:{};for(const k of ['mode','npc','evidenceFilter','evidenceSearch','caseTab'])if(v.ui[k]!==undefined&&typeof v.ui[k]!=='string')throw Error('저장한 화면 상태가 손상되었습니다.');if(v.ui.scrolls&&Object.values(v.ui.scrolls).some(a=>!Array.isArray(a)||a.length!==2||a.some(n=>!Number.isFinite(n)||n<0)))throw Error('화면 스크롤 기록이 손상되었습니다.');validateProofDrafts(v.ui.proofDrafts,v.game);validateCompareDrafts(v.ui.compareDrafts,v.game);validateReaderState(v.ui,v.game);return v;}
...
function validateCompareDrafts(drafts,game){
 if(drafts===undefined)return;
 if(!drafts||typeof drafts!=='object'||Array.isArray(drafts)||Object.keys(drafts).length>32)throw Error('자료 비교 초안 형식이 손상되었습니다.');
 for(const [id,d] of Object.entries(drafts)){
  if(typeof id!=='string'||id.length>80||!d||typeof d!=='object'||Array.isArray(d)||!d.facts||typeof d.facts!=='object'||Array.isArray(d.facts))throw Error('자료 비교 초안이 손상되었습니다.');
  const rows=Object.entries(d.facts);if(rows.length>8)throw Error('자료 비교 선택이 너무 많습니다.');
  for(const [eid,index] of rows){const e=root.STORY.evidence[eid];if(!e||!game.evidence.includes(eid)||!(index===null||Number.isInteger(index)&&index>=0&&index<e.facts.length))throw Error('자료 비교 선택이 현재 자료와 일치하지 않습니다.');}
 }
}
function validateReaderState(u,g){
 ...
 if(u.loopTab!==undefined&&!['facts','claims','questions','memo'].includes(u.loopTab))throw Error('사건 수첩 탭 상태가 손상되었습니다.');
 if(u.advancedCase!==undefined&&typeof u.advancedCase!=='boolean')throw Error('사건 기록 보기 상태가 손상되었습니다.');
 if(u.pastJudgments!==undefined&&typeof u.pastJudgments!=='boolean')throw Error('추리 기록 펼침 상태가 손상되었습니다.');
 if(u.onboarding!==undefined){const allowed=['investigation','evidence','deduction','return'];if(!u.onboarding||typeof u.onboarding!=='object'||Array.isArray(u.onboarding)||Object.entries(u.onboarding).some(([k,v])=>!allowed.includes(k)||typeof v!=='boolean'))throw Error('처음 안내 상태가 손상되었습니다.');}
}
