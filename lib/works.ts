...
export const works: Work[] = [
  {
    ...
    platform: "실물 보드게임",
    synopsis:
      "몬스터들이 모여 사는 포포롱 마을. 작은 구조대의 아침을 준비하던 슬라임이 냉동고 안에서 얼어붙은 채 발견됩니다. 한동안 슬라임만 바라보던 대원들은 서로의 얼굴을 살피기 시작합니다.",
    ...
    record: {
      label: "진행 중인 펀딩",
      title: "머더미스터리 2종",
      detail: `${fundingSummaryPeriod(dg2)} · 텀블벅`,
    },
  },
  {
    slug: "professor-rest",
    ...
    platform: "실물 보드게임",
    synopsis:
      "청람대학교 연구실 구성원들은 프로젝트가 끝난 뒤 호숫가 연수원으로 향합니다. 모두가 함께 쉬기로 한 다음 날 아침, 박정호 교수는 호숫가 계단 아래에서 죽은 채 발견됩니다.",
  ...
  },
  {
    ...
    platform: "실물 보드게임",
    synopsis:
      "달빛조차 닿지 않는 지하 예배실. 굳게 잠긴 문이 열린 뒤 제단 위에서 교주가 숨진 채 발견됩니다. 가면을 쓴 네 사람은 각자의 비밀을 지키며 진짜 범인을 찾아야 합니다.",
    image: "/assets/works/snake-carnival-cover.webp",
    alt: "검은색 실물 패키지로 제작된 뱀이 죽은 축제",
    externalUrl: "https://tumblbug.com/projectdg0",
    externalLabel: "텀블벅 프로젝트",
    characteristics: [
      "폐쇄된 지하 예배실에서 시작되는 사건",
      "카드 소지와 조사 과정을 활용하는 진행",
      "네 명이 각자의 비밀을 가진 구성",
    ],
    record: {
      label: "펀딩 성공",
      title: "4인용 머더 미스터리 뱀이 죽은 축제",
      detail: formatFundingSummary(dg0),
    },
  },
  {
    ...
    platform: "UZU",
    synopsis:
      "다른 세계의 존재를 불러들여 재앙을 일으킨 잿빛 일족은 세상의 미움을 받게 됩니다. 오랜 시간이 흐른 뒤, 잿빛 머리 소녀가 직접 만든 네 인형의 눈에 붉은빛이 돕니다. 소녀는 하루 동안의 소망을 품고 쿠로에게 초대장을 맡깁니다.",
    ...
    characteristics: [
      "작품 속 규칙과 설정을 근거로 추리",
      "높은 추리 난이도와 복잡한 문장 구성",
      "작은 단서를 분석하고 조합하는 숙련자에게 권장",
    ],
    officialIntroduction: {
      ...
      characters: [
        ...
        {
          name: "노란 머리의 드워프",
          description: "큰 방패를 지닌 남성. 사건 당일 테이블을 배치합니다.",
        },
    ...
    record: {
      label: "앱 공개",
      title: "UZU 수록 시나리오",
      detail: "2025년 12월 31일 공개 · Project DullG 제작",
    },
  },
  {
    slug: "red-lab",
    ...
    platform: "실물 보드게임",
    synopsis:
      "괴인과의 전투를 마친 다음 날, 레인저들의 구심점인 레드가 잠긴 방에서 숨진 채 발견됩니다. 남은 네 레인저는 비밀 연구소를 조사하며 그날 밤의 일을 되짚습니다.",
    ...
    record: {
      label: "펀딩 성공",
      title: "깊은 서사와 맑은 추리, 머더미스터리 3종",
      detail: formatFundingSummary(dg1),
    },
  },
  {
    slug: "gourmet-master",
    ...
    platform: "실물 보드게임",
    synopsis:
      "왕실 요리사 선발 대회의 비공개 심사가 끝난 순간 왕실 요리장이 쓰러집니다. 현장에 있던 네 요리사는 접시와 요리에 남은 단서로 범인을 찾아야 합니다.",
...
export function getWorkStatus(work: Work, today: Date = new Date()): WorkStatus {
  if (work.status === "펀딩 중" && work.fundingId) {
    return getFundingStatus(getFundingProject(work.fundingId), today) === "진행 중"
      ? "펀딩 중"
      : "출시";
  }
  return work.status;
...
export const publishedWorks = (today: Date = new Date()) =>
  works.filter((work) => getWorkStatus(work, today) !== "펀딩 중");
export const currentWorks = (today: Date = new Date()) =>
  works.filter((work) => getWorkStatus(work, today) === "펀딩 중");
export const homeFeaturedWorks = works.filter((work) =>
  ["slime-soda", "professor-rest", "snake-carnival"].includes(work.slug),
);
