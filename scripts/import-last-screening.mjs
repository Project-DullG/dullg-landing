...
const extensions = { "image/webp": "webp", "audio/ogg": "ogg", "audio/mpeg": "mp3" };
...
const modules = ["assets", "story", "story-expansion", "case-rules", "editorial", "narrative-data", "narrative-patch", "observations", "model", "guide", "deduction", "comfort", "storage", "bgm-policy", "music-mixer", "sound", "portrait-bounds", "presentation-cues", "presentation", "app"];
const scripts = [...html.matchAll(/<script[^>]*>([\s\S]*?)<\/script>/g)];
if (scripts.length !== modules.length) throw new Error("Unexpected script order");
for (const [i, script] of scripts.entries()) {
  const name = modules[i] + ".js";
  await writeFile(new URL(name, out), script[1]);
...
await sharp(new URL("media/title_keyart.webp", out).pathname).resize({ width: 1440, withoutEnlargement: true }).webp({ quality: 84 }).toFile(new URL("cover.webp", out).pathname);
await writeFile(new URL("provenance.json", out), JSON.stringify({ version: "0.10.0", sourceSha256: hash, modules, assetCount: Object.keys(assets).length }, null, 2));
