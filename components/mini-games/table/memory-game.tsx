...
export function MemoryGame() {
  ...
  return (
    ...
    >
      <div className={styles.memory} role="group" aria-label="짝 맞추기 카드 열두 장">
        {s.cards.map((value, i) => {
          ...
          return (
            <button
              ...
              aria-label={`${i + 1}번 카드, ${face ? MEMORY_NAMES[value] : "뒤집힌 카드"}${matched ? ", 짝을 찾음" : ""}`}
              onClick={() => setGame((old) => ({ ...old, state: flipMemory(old.state, i) }))}
            >
              <span aria-hidden="true">{face ? MEMORY_SYMBOLS[value] : "◇"}</span>
            </button>
          );
        })}
      </div>
