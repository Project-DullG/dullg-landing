...
export async function generateMetadata() {
  return localizeMetadata(
    pageMetadata("/activity/ulleung-ecotourism-ai", {
    ...
    <PageFrame>
      <article className="activity-case">
        <header className="activity-case-hero shell">
          <div>
            <Kicker>{t("교육 활동 \u00B7 2026년 7월 4일")}</Kicker>
            <h1>
              {t("울릉군 생태관광")}
              <br />
              <em>{t("AI 교육")}</em>
          ...
          <div>
            <p>
              {t(
                "울릉군의 관광 자원을 정리하고 생성형 AI로 홍보 문구와 웹페이지를 직접 만든 하루 과정입니다.",
              )}
            </p>
            <dl>
              <div>
                <dt>{t("대상")}</dt>
                <dd>{t("울릉군 글로벌 생태관광 전문인재 양성 교육 참여자")}</dd>
              </div>
              <div>
                <dt>{t("일정")}</dt>
                <dd>{t("2026년 7월 4일")}</dd>
        ...
        <section className="activity-case-material">
          <div className="shell">
            <div>
              <Kicker>{t("수업 내용")}</Kicker>
              <h2>{t("관광 소재로 웹페이지 만들기")}</h2>
              <p>
                {t(
                  "관광 소재를 고르고, AI로 소개 문구와 이미지를 다듬은 뒤 공개 가능한 웹페이지로 완성했습니다.",
                )}
              </p>
            </div>
            <div>
              <Link href="/materials/ulleung-ecotourism-ai">
                <span>
                  <small>{t("강의와 실습 자료")}</small>
                  <strong>{t("수강자료 확인하기 →")}</strong>
                </span>
              </Link>
            </div>
          </div>
