  ...
  return (
    <form onSubmit={handleSubmit} className="dash-card">
      ...
      <button type="submit" className="dash-button" disabled={loading}>
        {t(loading ? "저장 중..." : student ? "수정" : "등록")}
      </button>
      {t(error && <p className="dash-error">{t(error)}</p>)}
    </form>
  );
}
